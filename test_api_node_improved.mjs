...
const sb = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
  global: {
    fetch: (url, options) => {
      // Increase timeout from default 10s to 20s for slower connections
      return fetch(url, {
        ...options,
        timeout: 20000
      });
...
async function checkTable(name, queryFn, retries = 2) {
  let lastError = null;
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (error) {
        console.error(`[${name}] ERROR (attempt ${attempt}/${retries})`, {
        ...
        lastError = error;
        if (attempt < retries) {
          console.log(`[${name}] Retrying in 2 seconds...`);
      ...
      console.error(`[${name}] EXCEPTION (attempt ${attempt}/${retries})`, {
...
async function run() {
  ...
  console.log(`Timeout: 20000ms (20s) - Set higher for slow/unreliable connections`);
  console.log(`Retries: 2 attempts per table\n`);

  const students = await checkTable('students', () => sb.from('students').select('*').limit(200), 2);
  await checkTable('faculty', () => sb.from('faculty').select('*').limit(50), 2);
  await checkTable('attendance', () => sb.from('attendance').select('*').limit(50), 2);
  await checkTable('notes', () => sb.from('notes').select('*').limit(50), 2);
  await checkTable('fee_structure', () => sb.from('fee_structure').select('*').limit(100), 2);
  await checkTable('fee_payments', () => sb.from('fee_payments').select('*').limit(50), 2);
  await checkTable('timetables', () => sb.from('timetables').select('*').limit(50), 2);
  await checkTable('exam_marks', () => sb.from('exam_marks').select('*').limit(50), 2);
  ...
  try {
    ...
    if (uploadedPath) {
      ...
      if (!deleteError) console.log('[storage:cleanup] OK');
    }
  }

  console.log('\n=== Node API Diagnostics End ===');
}
